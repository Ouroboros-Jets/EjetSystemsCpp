...
namespace Ouroboros::UI {
    ...
    void DrawButtonImage(const std::shared_ptr<Image> &image, const ImU32 tintNormal, const ImU32 tintHovered, const ImU32 tintPressed, const ImVec2 rectMin,
    ...
    void RenderWindowOuterBorders(ImGuiWindow *window) {
        ...
        if (g.Style.FrameBorderSize > 0 && !(window->Flags & ImGuiWindowFlags_NoTitleBar) && !window->DockIsActive) {
            ...
            window->DrawList->AddLine(ImVec2(window->Pos.x + border_size, y), ImVec2(window->Pos.x + window->Size.x - border_size, y), ImGui::GetColorU32(ImGuiCol_Border),
    ...
    bool UpdateWindowManualResize(ImGuiWindow *window, ImVec2 &newSize, ImVec2 &newPosition) {
        ...
        auto CalcResizePosSizeFromAnyCorner = [CalcWindowSizeAfterConstraint](ImGuiWindow *window, const ImVec2 &corner_target, const ImVec2 &corner_norm, ImVec2 *out_pos,
                                                                              ImVec2 *out_size) {
        ...
        // Extend outside window for hovering/resizing (maxxed with TouchPadding) and inside windows for
        ...
        if (/*(flags & ImGuiWindowFlags_NoResize) || */
            (flags & ImGuiWindowFlags_AlwaysAutoResize) || window->AutoFitFramesX > 0 || window->AutoFitFramesY > 0)
            return false;
        ...
        for (int resize_grip_n = 0; resize_grip_n < resize_grip_count; resize_grip_n++) {
            ...
            if (held && g.IO.MouseDoubleClicked[0] && resize_grip_n == 0) {
                ...
                const float x =
                        g.IO.MousePos.x - g.ActiveIdClickOffset.x + ImLerp(def.InnerDir.x * grip_hover_outer_size, def.InnerDir.x * -grip_hover_inner_size, def.CornerPosN.x);
                const float y =
                        g.IO.MousePos.y - g.ActiveIdClickOffset.y + ImLerp(def.InnerDir.y * grip_hover_outer_size, def.InnerDir.y * -grip_hover_inner_size, def.CornerPosN.y);
    ...
    void EndMenubar() {
        ...
        // Save horizontal position so next append can reuse it. This is
