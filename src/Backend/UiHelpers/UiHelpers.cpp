...
namespace Ouroboros::UI {
    ...
    void DrawButtonImage(const std::shared_ptr<Image> &imageNormal, const std::shared_ptr<Image> &imageHovered,
                         const std::shared_ptr<Image> &imagePressed, const ImU32 tintNormal,
    ...
    bool UpdateWindowManualResize(ImGuiWindow *window, ImVec2 &newSize, ImVec2 &newPosition) {
        ...
        for (int resize_grip_n = 0; resize_grip_n < resize_grip_count; resize_grip_n++) {
            ...
            if (held && g.IO.MouseDoubleClicked[0] && resize_grip_n == 0) {
                ...
                const float x = g.IO.MousePos.x - g.ActiveIdClickOffset.x + ImLerp(
                                        def.InnerDir.x * grip_hover_outer_size, def.InnerDir.x * -grip_hover_inner_size,
                                        def.CornerPosN.x);
                const float y = g.IO.MousePos.y - g.ActiveIdClickOffset.y + ImLerp(
                                        def.InnerDir.y * grip_hover_outer_size, def.InnerDir.y * -grip_hover_inner_size,
                                        def.CornerPosN.y);
    ...
    bool BeginMenubar(const ImRect &barRectangle) {
        ...
        ImRect clip_rect(
                ...
                IM_ROUND(
                        ImMax(bar_rect.Min.x + window->Pos.x, bar_rect.Max.x - ImMax(window->WindowRounding, window->
                            WindowBorderSize))), IM_ROUND(bar_rect.Max.y + window->Pos.y));
