...
static bool MultiPositionSwitch(const char *label, int *current_position, const int max_positions, float width = 50.0f,
                                float height = 50.0f, const std::vector<SwitchPosition> &positions = {}) {
    ...
    if (clicked) {
        ...
        float relative_y = (mouse_pos.y - bb.Min.y) / height;
    ...
    for (int i = 0; i < max_positions; i++) {
        float y_pos = bb.Max.y - 10 - (height - 20) * i / (max_positions - 1);
    ...
    float handle_y = bb.Max.y - 10 - (height - 20) * (*current_position) / (max_positions - 1);
